...
namespace GenBase.Timer
{
    ...
    public class Timer
    {
        ...
        )
        {
            Key = key;
            Duration = duration;
            IsUpdateRatio = isUpdateRatio;

            if (onComplete != null)
                _onCompleteEvents.Add(onComplete);

            if (onUpdate != null)
                _onUpdateEvents.Add(onUpdate);
        ...
        public bool IsUpdateRatio { get; set; }
        ...
        (
            string key,
            float duration,
            TimerCounting counting = TimerCounting.Up,
            Action onComplete = null,
            Action<float> onUpdate = null,
            bool isLooped = false,
            bool isUpdateRatio = false,
        ...
        )
        {
            ...
            var timer = new Timer(key, duration, counting, onComplete, onUpdate, isLooped, isUpdateRatio, useRealTime, autoDestroyOwner);
        ...
        public void ChangeStartTime(float duration)
        {
            _startTime -= duration;
        }
        ...
        {
            IsCompleted = true;
            _onCompleteEvents.ForEach(ac => ac?.Invoke());
            _onUpdateEvents.ForEach(ac => ac?.Invoke(0));
        ...
        public float GetTimeElapsed2()
        {
            ...
            return _timeElapsedBeforeCancel ?? _timeElapsedBeforePause ?? DateTime.Now.Second - _startTime;
        }
        ...
        private readonly List<Action> _onCompleteEvents = new List<Action>();
        private readonly List<Action<float>> _onUpdateEvents = new List<Action<float>>();
        ...
        {
            _onCompleteEvents.Add(action);
        }

        public void RemoveCompleteEvent(Action action)
        {
            _onCompleteEvents.Remove(action);
        }

        public void RemoveAllCompleteEvent()
        {
            _onCompleteEvents.Clear();
        }

        public void AddUpdateEvent(Action<float> action)
        {
            _onUpdateEvents.Add(action);
        }

        public void RemoveUpdateEvent(Action<float> action)
        {
            _onUpdateEvents.Remove(action);
        }

        public void RemoveAllUpdateEvent()
        {
        ...
        public void Update()
        {
            ...
            if (_onUpdateEvents != null && _onUpdateEvents.Count > 0)
            {
                var invokeParam = 0f;

                if (!IsUpdateRatio)
                {
                    if (Counting == TimerCounting.Up)
                        invokeParam = GetTimeElapsed();
                    else
                        invokeParam = GetTimeRemaining();
                }
                else
                {
                    invokeParam = GetRatioRemaining();
                }

                _onUpdateEvents.ForEach(ac => ac?.Invoke(invokeParam));
            }

            if (GetWorldTime() >= GetFireTime())
            {
                if (_onCompleteEvents != null && _onUpdateEvents.Count > 0)
                    _onCompleteEvents.ForEach(ac => ac?.Invoke());
